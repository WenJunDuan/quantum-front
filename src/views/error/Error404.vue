...
Principle: Missing routes should land on a clear page.
...
const message = computed(() => {
  if (lastError.value?.code === 404 && lastError.value.message) return lastError.value.message
  return "你访问的页面不存在。"
})

const traceId = computed(() =>
  lastError.value?.code === 404 ? (lastError.value.traceId ?? null) : null,
)
