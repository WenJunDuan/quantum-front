...
Principle: Fatal errors must be obvious and actionable.
...
const message = computed(() => {
  if (lastError.value?.code === 400 && lastError.value.message) return lastError.value.message
  return "请求参数或业务处理异常，请检查后重试。"
})

const traceId = computed(() =>
  lastError.value?.code === 400 ? (lastError.value.traceId ?? null) : null,
)
